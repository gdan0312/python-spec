...
class ServerSocket:
    ...
    def send(self, data):
        if data in self.rsp_map:
            ...
            raise ServerSocketException("запрос не соответствует протоколу: {data}".format(data=data))
...
class TestClient(unittest.TestCase):
    ...
    def test_client_get_client_error(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Некорректно обработано сообщение сервера об ошибке: {message}".format(message=message))
